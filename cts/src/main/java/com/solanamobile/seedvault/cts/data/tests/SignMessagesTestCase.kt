...
internal class SignMaxMessageWithMaxSignatureTestCase @Inject constructor(
    ...
    init {
        check(implementationDetails.MAX_SIGNING_REQUESTS <= 10) { "Test case implementation currently only handles up to 3 messages per signing request" }
        check(implementationDetails.MAX_REQUESTED_SIGNATURES <= 3) { "Test case implementation currently only handles up to 3 signatures per message" }
    }
}
